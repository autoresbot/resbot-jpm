...
async function tanyaMetodeLogin() {
  ...
  const tanya = (pertanyaan) => new Promise((resolve) => rl.question(pertanyaan, (j) => resolve(j.trim())));

  try {
    while (true) {
      const metode = (await tanya(clc.yellow.bold('Pilih metode koneksi (qr/pairing): '))).toLowerCase();

      if (metode === 'qr') return { metode };

      if (metode === 'pairing') {
        const nomor = (await tanya(clc.yellow.bold('Masukkan nomor WhatsApp (contoh 628xxx): '))).replace(/\D/g, '');
